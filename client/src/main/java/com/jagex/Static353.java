package com.jagex;

import org.openrs2.deob.annotation.OriginalMember;

public final class Static353 {

	@OriginalMember(owner = "com.jagex.client!ui", name = "j", descriptor = "I")
	public static int anInt6532;

	@OriginalMember(owner = "com.jagex.client!ui", name = "q", descriptor = "[I")
	public static int[] anIntArray450;

	@OriginalMember(owner = "com.jagex.client!ui", name = "a", descriptor = "Lclient!ad;")
	public static final HashTable aClass4_136 = new HashTable(32);

	@OriginalMember(owner = "com.jagex.client!ui", name = "m", descriptor = "[I")
	public static int[] anIntArray448 = new int[2];

	@OriginalMember(owner = "com.jagex.client!ui", name = "n", descriptor = "[I")
	public static final int[] anIntArray449 = new int[] { 104, 120, 136, 168 };

	@OriginalMember(owner = "com.jagex.client!ui", name = "b", descriptor = "(I)V")
	public static void method5883() {
		if (Rasteriser.instance.method2859()) {
			Static296.method5104();
			Rasteriser.instance.method2828(GameShell.canvas);
			Static98.method2107();
		} else {
			Static78.method5701(Static77.anInt1762);
		}
	}
}
