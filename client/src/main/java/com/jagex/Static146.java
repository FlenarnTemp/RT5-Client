package com.jagex;

import org.openrs2.deob.annotation.OriginalMember;

public final class Static146 {

	@OriginalMember(owner = "com.jagex.client!ih", name = "b", descriptor = "Lclient!gd;")
	public static final LocalizedString aClass79_63 = new LocalizedString("Allocated memory", "Zugewiesener Speicher.", "Mémoire attribuée", "Memória alocada");

	@OriginalMember(owner = "com.jagex.client!ih", name = "f", descriptor = "I")
	public static int anInt2703 = 0;

	@OriginalMember(owner = "com.jagex.client!ih", name = "j", descriptor = "I")
	public static int anInt2706 = 0;

}
